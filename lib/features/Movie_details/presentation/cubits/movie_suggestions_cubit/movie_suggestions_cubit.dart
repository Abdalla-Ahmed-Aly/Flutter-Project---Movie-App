...
class MovieSuggestionsCubit extends Cubit<MovieSuggestionsState> {
  ...
  MovieSuggestionsCubit({required this.movieSuggestionsRepository})
      : super(MovieSuggestionsInitial());
  Future<void> loadMovieSuggestions(int movieId) async {
    emit(MovieSuggestionsLoading());
    try {
      final movieSuggestionsList =
          await movieSuggestionsRepository.getMovieSuggestionsList(movieId);
